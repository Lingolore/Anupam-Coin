 ...
 */
export async function ultraSensitiveTest(): Promise<void> {
    ...
    try {
        ...
        const baselineResponse: PriceResponse = await fetchPrices();
        ...
        const originalBTC :any= basePriceData.crypto.find(c => c.symbol === 'BTC/USD');
        const originalETH:any = basePriceData.crypto.find(c => c.symbol === 'ETH/USD');
